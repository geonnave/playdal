...
class SpotifyView extends Component {
  state = {
    inputPlaylistURL: "https://open.spotify.com/user/geonnave/playlist/5TVPDoTAhcY1005jhBvZPz?si=2zS6BdfBQxmpGN9dnAeG_w",
    activePlaylistURL: "",
  ...
  URIdidNotChanged = () => {
    return this.URLtoURI(this.state.inputPlaylistURL) == this.URLtoURI(this.state.activePlaylistURL)
  }

  validatePlaylist = (playlistURL) => (
    /https:\/\/open.spotify.com\/user\/.*\/playlist\/[a-zA-Z0-9]+/.test(playlistURL)
  )

  playPlaylist = () => {
    if (!this.validatePlaylist(this.state.inputPlaylistURL)) {
      console.log(`Not playing invalid playlist URL: ${this.state.inputPlaylistURL}`)
      return;
    }
    Spotify.getPlaybackStateAsync().then((playbackState) => {
      ...
      if (this.URIdidNotChanged()) {
        ...
        this.setState({...this.state, activePlaylistURL: this.state.inputPlaylistURL})
  ...
  spotifyInitialize = () => {
    let spotifyOptions = {
      ...
      "scopes": ["user-read-private", "playlist-read", "playlist-read-private", "streaming", "app-remote-control"]
    }
